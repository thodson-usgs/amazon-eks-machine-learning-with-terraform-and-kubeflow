...
class VLLMDeployment:
    ...
    async def _create_openai_handlers(self):
        ...
        async with self._init_lock:
            ...
            # Initialize serving render (required by OpenAIServingChat in vLLM 0.19.0)
            print("Initialize serving render")
            self._serving_render = OpenAIServingRender(
                model_config=self._engine_client.model_config,
                renderer=self._engine_client.renderer,
                io_processor=self._engine_client.io_processor,
                model_registry=self._serving_models.registry,
            ...
            self._serving_chat = OpenAIServingChat(
                ...
                openai_serving_render=self._serving_render,
                request_logger=request_logger,
                chat_template=resolved_chat_template,
                chat_template_content_format=chat_template_content_format,
            ...
            self._serving_embedding = ServingEmbedding(
                engine_client=self._engine_client,
                models=self._serving_models,
                request_logger=request_logger,
                chat_template=resolved_chat_template,
