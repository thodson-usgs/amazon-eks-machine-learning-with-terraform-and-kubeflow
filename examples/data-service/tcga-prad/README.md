# TCGA Preprocessing Example: PRAD

This example demonstrates how to preprocess TCGA (The Cancer Genome Atlas) data for Prostate Adenocarcinoma using the TCGA Helm chart.
...
- Sufficient storage (~15-25 GB for PRAD project)
...
```bash
cd examples/data-service/tcga-prad
bash deploy.sh
```
...
```bash
# Watch job status
kubectl get jobs -n kubeflow -l app.kubernetes.io/instance=tcga-prad -w

# View logs
kubectl logs -n kubeflow -l app.kubernetes.io/instance=tcga-prad --follow

# Check pod status
kubectl get pods -n kubeflow -l app.kubernetes.io/instance=tcga-prad
```
...
```
/fsx/home/tcga-prad/tcga_processed/
├── TCGA-PRAD/
│   ├── clinical.parquet
...
```bash
helm install tcga-prad ../../../charts/data-service/tcga \
  --namespace kubeflow \
  --values values.yaml
```
...
- **Total**: ~15-30 minutes for PRAD project
...
```python
import pandas as pd
clinical = pd.read_parquet("/fsx/home/tcga-prad/tcga_processed/TCGA-PRAD/clinical.parquet")
...
```python
expr = pd.read_parquet("/fsx/home/tcga-prad/tcga_processed/TCGA-PRAD/expression_matrix.parquet")
print(expr.shape)  # (60000 genes, 550 samples)
```
...
```python
mutations = pd.read_parquet("/fsx/home/tcga-prad/tcga_processed/TCGA-PRAD/mutations.parquet")
...
expr = pd.read_parquet("/fsx/home/tcga-prad/tcga_processed/TCGA-PRAD/expression_matrix.parquet")
genes = pd.read_parquet("/fsx/home/tcga-prad/tcga_processed/metadata/gene_id_mapping.parquet")

# Get AR expression (androgen receptor, key in prostate cancer)
ar_id = genes[genes['gene_name'] == 'AR']['gene_id'].iloc[0]
ar_expr = expr.loc[ar_id]

# Load mutations
mutations = pd.read_parquet("/fsx/home/tcga-prad/tcga_processed/TCGA-PRAD/mutations.parquet")
spop_muts = mutations[mutations['Hugo_Symbol'] == 'SPOP']
```
