# TCGA Preprocessing Example: COAD

This example demonstrates how to preprocess TCGA (The Cancer Genome Atlas) data for Colon Adenocarcinoma using the TCGA Helm chart.
...
- Sufficient storage (~15-25 GB for COAD project)
...
```bash
cd examples/data-service/tcga-coad
bash deploy.sh
```
...
```bash
# Watch job status
kubectl get jobs -n kubeflow -l app.kubernetes.io/instance=tcga-coad -w

# View logs
kubectl logs -n kubeflow -l app.kubernetes.io/instance=tcga-coad --follow

# Check pod status
kubectl get pods -n kubeflow -l app.kubernetes.io/instance=tcga-coad
```
...
```
/fsx/home/tcga-coad/tcga_processed/
├── TCGA-COAD/
│   ├── clinical.parquet
...
```bash
helm install tcga-coad ../../../charts/data-service/tcga \
  --namespace kubeflow \
  --values values.yaml
```
...
- **Total**: ~15-30 minutes for COAD project
...
```python
import pandas as pd
clinical = pd.read_parquet("/fsx/home/tcga-coad/tcga_processed/TCGA-COAD/clinical.parquet")
...
```python
expr = pd.read_parquet("/fsx/home/tcga-coad/tcga_processed/TCGA-COAD/expression_matrix.parquet")
print(expr.shape)  # (60000 genes, 500 samples)
```
...
```python
mutations = pd.read_parquet("/fsx/home/tcga-coad/tcga_processed/TCGA-COAD/mutations.parquet")
...
expr = pd.read_parquet("/fsx/home/tcga-coad/tcga_processed/TCGA-COAD/expression_matrix.parquet")
genes = pd.read_parquet("/fsx/home/tcga-coad/tcga_processed/metadata/gene_id_mapping.parquet")

# Get APC expression (commonly mutated in colon cancer)
apc_id = genes[genes['gene_name'] == 'APC']['gene_id'].iloc[0]
apc_expr = expr.loc[apc_id]

# Load mutations
mutations = pd.read_parquet("/fsx/home/tcga-coad/tcga_processed/TCGA-COAD/mutations.parquet")
kras_muts = mutations[mutations['Hugo_Symbol'] == 'KRAS']
```
